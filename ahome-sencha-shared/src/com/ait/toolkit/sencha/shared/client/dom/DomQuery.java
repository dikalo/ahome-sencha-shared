...
public class DomQuery {
	...
	private static native JavaScriptObject doCompile(String selector,
			String selectorType) /*-{
	 ...
	 */
	public static Element[] filter(Element[] els, String selector,
			boolean nonMatches) {
		JavaScriptObject jsElements = doFilter(
				JsoHelper.convertToJavaScriptArray(els), selector, nonMatches);
		return JsoHelper.toElementArray(jsElements);
	}

	private static native JavaScriptObject doFilter(JavaScriptObject els,
			String selector, boolean nonMatches) /*-{
