...
public class Anim extends JsObject {
	...
	private IsComponentElement target;
	...
	public Anim(IsComponentElement target) {
		this();
		setTarget(target);
	}
	...
	public native void setCallback(Function callback)/*-{
		var c = this.@com.ait.toolkit.sencha.shared.client.fx.Anim::getJsObj()();
		c.callback = function() {
	...
	public double getCurrentIteration() {
		if (jsObj == null) {
			throw new IllegalStateException(
					"Can not get currentIteration. Did you can run() on this Anim Config ? ");
	 ...
	 * The element to apply the animation to.
	 */
	public void setTarget(IsComponentElement target) {
		this.target = target;
