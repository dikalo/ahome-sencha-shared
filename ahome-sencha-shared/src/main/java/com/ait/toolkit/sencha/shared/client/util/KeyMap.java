...
public class KeyMap extends JsObject {
	...
	private static native JavaScriptObject create(Element el,
			JavaScriptObject config, String eventName) /*-{
		return new $wnd.Ext.KeyMap(el, config, eventName);
	}-*/;

	private static native JavaScriptObject create(String id,
			JavaScriptObject config, String eventName) /*-{
	...
	public native void addBinding(KeyMapConfig config) /*-{
		var km = this.@com.ait.toolkit.core.client.JsObject::getJsObj()();
		var configJS = this.@com.ait.toolkit.core.client.JsObject::getJsObj()();
